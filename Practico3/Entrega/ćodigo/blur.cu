...
__global__ void ej1b_no_div_kernel(float* o_img, float* d_img, int width, int height, float coef){
    ...
    // Mantenemos este for por consistencia con el criterio adoptado en el comentario [*1]
    d_img[pos]=o_img[pos];
    float coefAux = coef / 10.0f;
    for(int i=0;i<ITERATIONS;i++){
        d_img[pos]=max(0.0f , min(255.0f,d_img[pos] + (1 - 2 * par)*coefAux));
    }
}
...
__global__ void ej1b_div_kernel(float* o_img, float* d_img, int width, int height, float coef){
    ...
    // [*1] Aumentamos el iteraciones dentro de la divergencia para hacer su efecto mas notorio
    d_img[pos]=o_img[pos];
    if (par == 0) {
        for(int i=0;i<ITERATIONS;i++){
            d_img[pos]=max(0.0f, min(255.0f,d_img[pos] + coefAux));
        }
    } else {
        for(int i=0;i<ITERATIONS;i++){
            d_img[pos]=max(0.0f , min(255.0f,d_img[pos] - coefAux));
